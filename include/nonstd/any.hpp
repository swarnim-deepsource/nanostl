...
// Studio 6.0) MSVC++  7.0  _MSC_VER == 1300  any_COMPILER_MSVC_VERSION ==  70
// (Visual Studio .NET 2002) MSVC++  7.1  _MSC_VER == 1310
// any_COMPILER_MSVC_VERSION ==  71  (Visual Studio .NET 2003) MSVC++  8.0
// _MSC_VER == 1400  any_COMPILER_MSVC_VERSION ==  80  (Visual Studio 2005)
// MSVC++  9.0  _MSC_VER == 1500  any_COMPILER_MSVC_VERSION ==  90  (Visual
// Studio 2008) MSVC++ 10.0  _MSC_VER == 1600  any_COMPILER_MSVC_VERSION == 100
// (Visual Studio 2010) MSVC++ 11.0  _MSC_VER == 1700  any_COMPILER_MSVC_VERSION
// == 110  (Visual Studio 2012) MSVC++ 12.0  _MSC_VER == 1800
// any_COMPILER_MSVC_VERSION == 120  (Visual Studio 2013) MSVC++ 14.0  _MSC_VER
// == 1900  any_COMPILER_MSVC_VERSION == 140  (Visual Studio 2015) MSVC++ 14.1
// _MSC_VER >= 1910  any_COMPILER_MSVC_VERSION == 141  (Visual Studio 2017)
...
#define any_REQUIRES_R(R, ...) typename nanostl::enable_if<__VA_ARGS__, R>::type
...
class any {
 public:
  ...
  any(any&& other) any_noexcept : content(nanostl::move(other.content)) {
    other.content = any_nullptr;
  }

  template <class ValueType,
            class T = typename nanostl::decay<ValueType>::type any_REQUIRES_T(
                !nanostl::is_same<T, any>::value)>
  ...
  template <class T, class U,
            class... Args any_REQUIRES_T(
                nanostl::is_constructible<T, nanostl::initializer_list<U>&,
                                          Args&&...>::value)>
  explicit any(nonstd_lite_in_place_type_t(T), nanostl::initializer_list<U> il,
               Args&&... args)
  ...
  void swap(any& other) any_noexcept { nanostl::swap(content, other.content); }
...
#if any_CPP11_OR_GREATER
    holder(ValueType&& value) : held(nanostl::move(value)) {}
#endif
...
template <class ValueType
          ...
          ,
          typename = typename nanostl::enable_if<
              (nanostl::is_reference<ValueType>::value ||
               nanostl::is_copy_constructible<ValueType>::value),
              any_lite::detail::enabler>::type
#endif
          >
...
template <class ValueType
#if any_HAVE_DEFAULT_FUNCTION_TEMPLATE_ARG
              any_REQUIRES_T(nanostl::is_reference<ValueType>::value ||
                             nanostl::is_copy_constructible<ValueType>::value)
#endif
          >
any_nodiscard inline ValueType any_cast(any&& operand) {
...
template <class ValueType>
any_nodiscard inline ValueType const* any_cast(any const* operand)
    any_noexcept {
