...
//
// in_place: code duplicated in any-lite, expected-lite, expected-lite,
...
// Compiler versions:
//
// MSVC++  6.0  _MSC_VER == 1200  nsel_COMPILER_MSVC_VERSION ==  60  (Visual
// Studio 6.0) MSVC++  7.0  _MSC_VER == 1300  nsel_COMPILER_MSVC_VERSION ==  70
// (Visual Studio .NET 2002) MSVC++  7.1  _MSC_VER == 1310
// nsel_COMPILER_MSVC_VERSION ==  71  (Visual Studio .NET 2003) MSVC++  8.0
// _MSC_VER == 1400  nsel_COMPILER_MSVC_VERSION ==  80  (Visual Studio 2005)
// MSVC++  9.0  _MSC_VER == 1500  nsel_COMPILER_MSVC_VERSION ==  90  (Visual
// Studio 2008) MSVC++ 10.0  _MSC_VER == 1600  nsel_COMPILER_MSVC_VERSION == 100
// (Visual Studio 2010) MSVC++ 11.0  _MSC_VER == 1700 nsel_COMPILER_MSVC_VERSION
// == 110  (Visual Studio 2012) MSVC++ 12.0  _MSC_VER == 1800
// nsel_COMPILER_MSVC_VERSION == 120  (Visual Studio 2013) MSVC++ 14.0  _MSC_VER
// == 1900  nsel_COMPILER_MSVC_VERSION == 140  (Visual Studio 2015) MSVC++ 14.1
// _MSC_VER >= 1910  nsel_COMPILER_MSVC_VERSION == 141  (Visual Studio 2017)
// MSVC++ 14.2  _MSC_VER >= 1920  nsel_COMPILER_MSVC_VERSION == 142  (Visual
// Studio 2019)
...
// #define nsel_CPP11_140  (nsel_CPP11_OR_GREATER || nsel_COMPILER_MSVC_VER >=
// 1900)
...
// - C26409: Avoid calling new and delete explicitly, use
// nanostl::make_unique<T> instead (r.11)
...
nsel_DISABLE_MSVC_WARNINGS(26409)
  ...
  template <typename...>
  struct conjunction : nanostl::true_type {};
  template <typename B1>
  struct conjunction<B1> : B1 {};

  template <typename B1, typename... Bn>
  struct conjunction<B1, Bn...>
      : nanostl::conditional<bool(B1::value), conjunction<Bn...>, B1>::type {};
  ...
  template <typename T>
  struct remove_cvref {
    typedef typename nanostl::remove_cv<
        typename nanostl::remove_reference<T>::type>::type type;
  };
  ...
  class storage_t_impl {
    ...
    friend class expected_lite::expected;
   ...
   public:
    ...
    nsel_constexpr14 value_type &&value() && { return nanostl::move(m_value); }
    ...
    nsel_constexpr14 error_type &&error() && { return nanostl::move(m_error); }
  ...
  class storage_t<T, E, true, true> : public storage_t_impl<T, E> {
   public:
    ...
    storage_t(storage_t &&other) : storage_t_impl<T, E>(other.has_value()) {
      if (this->has_value())
        this->construct_value(nanostl::move(other.value()));
      else
        this->construct_error(nanostl::move(other.error()));
    }
  };
  ...
  /// x.x.5 Unexpected object type; unexpected_type; C++17 and later can also
  /// use aliased type unexpected.
...
#endif  // nsel_P0323R
  {
   public:
    ...
    constexpr explicit unexpected_type(nonstd_lite_in_place_t(E),
                                       Args &&...args)
        : m_error(nanostl::forward<Args>(args)...) {}

    template <typename U,
              typename... Args nsel_REQUIRES_T(
                  nanostl::is_constructible<E, nanostl::initializer_list<U>,
                                            Args &&...>::value)>
    constexpr explicit unexpected_type(nonstd_lite_in_place_t(E),
                                       nanostl::initializer_list<U> il,
                                       Args &&...args)
        : m_error(il, nanostl::forward<Args>(args)...) {}

    template <typename E2 nsel_REQUIRES_T(
        nanostl::is_constructible<E, E2>::value &&
        !nanostl::is_same<typename std20::remove_cvref<E2>::type,
    ...
    nsel_constexpr14 E &&value() && noexcept { return nanostl::move(m_error); }
    ...
    nsel_REQUIRES_R(void, std17::is_swappable<E>::value)
        swap(unexpected_type &other)
            noexcept(std17::is_nothrow_swappable<E>::value) {
      using nanostl::swap;
  ...
  // TODO: Should expected be specialized for particular E types such as
  // exception_ptr and how?
  ...
  class unexpected_type<nanostl::exception_ptr> {
   public:
    ...
    nanostl::exception_ptr const &value() const { return m_error; }

    nanostl::exception_ptr &value() { return m_error; }
  ...
  constexpr bool operator==(unexpected_type<E1> const &x,
                            unexpected_type<E2> const &y) {
  ...
  template <typename E>
  constexpr bool operator<(unexpected_type<E> const &x,
  ...
  void swap(unexpected_type<E> &x, unexpected_type<E> &y)
  ...
  inline constexpr bool operator<(
      unexpected_type<nanostl::exception_ptr> const & /*x*/,
      unexpected_type<nanostl::exception_ptr> const & /*y*/) {
    return false;
  }
  ...
  nsel_constexpr14 auto make_unexpected(E &&value)
      -> unexpected_type<typename nanostl::decay<E>::type> {
    return unexpected_type<typename nanostl::decay<E>::type>(
        nanostl::forward<E>(value));
  }

#if nsel_P0323R <= 3

  /*nsel_constexpr14*/ auto inline make_unexpected_from_current_exception()
      -> unexpected_type<nanostl::exception_ptr> {
    return unexpected_type<nanostl::exception_ptr>(
        nanostl::current_exception());
  }
  ...
  class bad_expected_access<void> : public nanostl::exception {
   public:
    explicit bad_expected_access() : nanostl::exception() {}
  };
  ...
  struct error_traits<nanostl::exception_ptr> {
    static void rethrow(nanostl::exception_ptr const & /*e*/) {
      ...
      assert(false &&
             detail::text(
                 "throw bad_expected_access<nanostl::exception_ptr>{ e };"));
#endif
    }
  };
...
#endif  // nsel_P0323R
  {
   ...
   public:
    ...
    nsel_REQUIRES_0(nanostl::is_default_constructible<T>::value)
    ...
    template <
        ...
        typename G nsel_REQUIRES_T(
            ...
            !nanostl::is_convertible<expected<U, G> const &&, T>::value &&
            (!nanostl::is_convertible<U const &, T>::value ||
    ...
    nsel_constexpr14 explicit expected(expected<U, G> &&other)
      ...
      if (has_value())
        ...
        contained.construct_error(E{nanostl::move(other.contained.error())});
    }
    ...
    nsel_constexpr14 /*non-explicit*/ expected(expected<U, G> &&other)
      ...
      if (has_value())
        ...
        contained.construct_error(nanostl::move(other.contained.error()));
    }

    template <typename U =
                  T nsel_REQUIRES_T(nanostl::is_copy_constructible<U>::value)>
    ...
    template <
        typename U = T nsel_REQUIRES_T(
            nanostl::is_constructible<T, U &&>::value &&
            !nanostl::is_same<typename std20::remove_cvref<U>::type,
                              nonstd_lite_in_place_t(U)>::value &&
            !nanostl::is_same<expected<T, E>,
                              typename std20::remove_cvref<U>::type>::value &&
            !nanostl::is_same<nanostl::unexpected_type<E>,
                              typename std20::remove_cvref<U>::type>::value &&
            !nanostl::is_convertible<U &&, T>::value /*=> explicit */
            )>
    nsel_constexpr14 explicit expected(U &&value)
        noexcept(nanostl::is_nothrow_move_constructible<U>::value
    ...
    template <typename G = E nsel_REQUIRES_T(
                  nanostl::is_constructible<E, G const &>::value &&nanostl::
                      is_convertible<G const &, E>::value /*=> non-explicit */
                  )>
    nsel_constexpr14 /*non-explicit*/ expected(
        nanostl::unexpected_type<G> const &error)
        : contained(false) {
    ...
    nsel_constexpr14 /*non-explicit*/ expected(
        nanostl::unexpected_type<G> &&error)
        : contained(false) {
    ...
    nsel_constexpr14 explicit expected(nonstd_lite_in_place_t(T),
                                       Args &&...args)
        : contained(true) {
    ...
    template <typename U,
              typename... Args nsel_REQUIRES_T(
                  nanostl::is_constructible<E, nanostl::initializer_list<U>,
                                            Args &&...>::value)>
    nsel_constexpr14 explicit expected(unexpect_t,
                                       nanostl::initializer_list<U> il,
                                       Args &&...args)
        : contained(false) {
    ...
    // false and !bool(*this), calls unexpect.~unexpected<E>(). Remarks: If
    // either T is cv void or is_trivially_destructible_v<T> is true, and
    ...
    expected &operator=(expected &&other) noexcept(
        nanostl::is_nothrow_move_constructible<
            T>::value &&nanostl::is_nothrow_move_assignable<T>::value
            &&nanostl::is_nothrow_move_constructible<
                E>::value  // added for missing
                &&nanostl::is_nothrow_move_assignable<E>::value)  //   nothrow
                                                                  //   above
    {
    ...
    template <typename U nsel_REQUIRES_T(
        !nanostl::is_same<expected<T, E>,
                          typename std20::remove_cvref<U>::type>::value &&
        std17::conjunction<nanostl::is_scalar<T>,
                           nanostl::is_same<T, nanostl::decay<U> > >::value &&
        nanostl::is_constructible<T, U>::value &&
    ...
    template <typename G nsel_REQUIRES_T(
        nanostl::is_copy_constructible<
            E>::value  // TODO: nanostl::is_nothrow_copy_constructible<E>
    ...
    template <typename G nsel_REQUIRES_T(
        nanostl::is_move_constructible<
            E>::value  // TODO: nanostl::is_nothrow_move_constructible<E>
    ...
    value_type &emplace(Args &&...args) {
      expected(nonstd_lite_in_place(T), nanostl::forward<Args>(args)...)
          .swap(*this);
      return value();
    }
    ...
    template <typename U,
              typename... Args nsel_REQUIRES_T(
                  nanostl::is_nothrow_constructible<
                      T, nanostl::initializer_list<U> &, Args &&...>::value)>
    value_type &emplace(nanostl::initializer_list<U> il, Args &&...args) {
      expected(nonstd_lite_in_place(T), il, nanostl::forward<Args>(args)...)
          .swap(*this);
      return value();
    }
    ...
    nsel_REQUIRES_R(
        void, std17::is_swappable<U>::value &&std17::is_swappable<G>::value &&
                  (nanostl::is_move_constructible<U>::value ||
                   nanostl::is_move_constructible<G>::value))
        swap(expected &other)
            noexcept(nanostl::is_nothrow_move_constructible<T>::value
      ...
      if (bool(*this) && bool(other)) {
        ...
        error_type t(nanostl::move(other.error()));
    ...
    bool has_exception() const {
      using ContainedEx = typename nanostl::remove_reference<decltype(
          get_unexpected().value())>::type;
    ...
    value_type value_or(U &&v) && {
      return has_value() ? nanostl::move(contained.value())
                         : static_cast<T>(nanostl::forward<U>(v));
    }
  ...
  class expected<void, E> {
   ...
   public:
    ...
    expected &operator=(expected &&other)
        noexcept(nanostl::is_nothrow_move_assignable<E>::value
                     &&nanostl::is_nothrow_move_constructible<E>::value) {
      expected(nanostl::move(other)).swap(*this);
      return *this;
    }
    ...
    nsel_REQUIRES_R(void, std17::is_swappable<G>::value
                              &&nanostl::is_move_constructible<G>::value)
        swap(expected &other)
            noexcept(nanostl::is_nothrow_move_constructible<E>::value
                         &&std17::is_nothrow_swappable<E &>::value) {
      using nanostl::swap;
      ...
      if (!bool(*this) && !bool(other)) {
        ...
        contained.construct_error(nanostl::move(other.error()));
        bool has_value = contained.has_value();
        bool other_has_value = other.has_value();
        other.contained.set_has_value(has_value);
        contained.set_has_value(other_has_value);
   ...
   private:
    detail::storage_t<void, E, nanostl::is_copy_constructible<E>::value,
                      nanostl::is_move_constructible<E>::value>
        contained;
  };
  ...
  constexpr bool operator==(expected<T1, E1> const &x,
                            expected<T2, E2> const &y) {
    return bool(x) != bool(y) ? false
           : bool(x) == false ? x.error() == y.error()
                              : *x == *y;
  }
  ...
  template <typename E1, typename E2>
  constexpr bool operator==(expected<void, E1> const &x,
                            expected<void, E1> const &y) {
    return bool(x) != bool(y) ? false
           : bool(x) == false ? x.error() == y.error()
                              : true;
  }
  ...
  template <typename T1, typename E1, typename E2>
  constexpr bool operator==(unexpected_type<E2> const &u,
  ...
  template <typename T, typename E>
  constexpr bool operator<(expected<T, E> const &x,
                           unexpected_type<E> const &u) {
    return (!x) ? (x.get_unexpected() < u) : false;
  }

  template <typename T, typename E>
  constexpr bool operator<(unexpected_type<E> const &u,
  ...
  template <typename T,
            typename E nsel_REQUIRES_T(
                (nanostl::is_void<T>::value ||
                 nanostl::is_move_constructible<T>::value) &&
                nanostl::is_move_constructible<E>::value &&
                std17::is_swappable<T>::value && std17::is_swappable<E>::value)>
  ...
  constexpr auto make_expected(T &&v)
      -> expected<typename nanostl::decay<T>::type> {
  ...
  auto make_expected_from_exception(nanostl::exception_ptr v) -> expected<T> {
    return expected<T>(unexpected_type<nanostl::exception_ptr>(
        nanostl::forward<nanostl::exception_ptr>(v)));
  }

  template <typename T, typename E>
  constexpr auto make_expected_from_error(E e)
      -> expected<T, typename nanostl::decay<E>::type> {
    return expected<T, typename nanostl::decay<E>::type>(make_unexpected(e));
  }

  template <typename F nsel_REQUIRES_T(
      !nanostl::is_same<typename nanostl::result_of<F()>::type, void>::value)>
  /*nsel_constexpr14*/
  auto make_expected_from_call(F f)
      -> expected<typename nanostl::result_of<F()>::type> {
    try {
