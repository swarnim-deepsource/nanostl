...
#define nonstd_lite_in_place_type(T) \
  nanostl::in_place_type_t<T> {}
#define nonstd_lite_in_place_index(K) \
  nanostl::in_place_index_t<K> {}
...
//
// Use C++17 nanostl::variant:
//

#if variant_USES_STD_VARIANT

#include <functional>  // nanostl::hash<>
#include <variant>
...
using nanostl::bad_variant_access;
using nanostl::hash;
using nanostl::monostate;
using nanostl::variant;
using nanostl::variant_alternative;
using nanostl::variant_alternative_t;
using nanostl::variant_size;
using nanostl::variant_size_v;

using nanostl::get;
using nanostl::get_if;
using nanostl::holds_alternative;
using nanostl::visit;
...
constexpr auto variant_npos = nanostl::variant_npos;
}  // namespace nonstd
...
#include <nanofunctional.h>  // nanostl::hash
#endif
...
#define variant_REQUIRES_0(...)     \
  template <bool B = (__VA_ARGS__), \
            typename nanostl::enable_if<B, int>::type = 0>
...
using nanostl::add_pointer;

#elif variant_HAVE_TR1_ADD_POINTER

using nanostl::tr1::add_pointer;
...
struct is_swappable {
  template <typename T, typename = decltype(swap(nanostl::declval<T&>(),
                                                 nanostl::declval<T&>()))>
  static nanostl::true_type test(int);
...
struct is_nothrow_swappable {
  ...
  template <typename T>
  static auto test(int) -> nanostl::integral_constant<bool, test<T>()> {}
...
struct typelist_max<typelist<Head, Tail> > {
 ...
 public:
  enum V {
    value =
        (sizeof(Head) > tail_value) ? sizeof(Head) : nanostl::size_t(tail_value)
  };
...
struct typelist_max_alignof<typelist<Head, Tail> > {
 ...
 public:
  enum V {
    value = (alignof(Head) > tail_value) ? alignof(Head)
                                         : nanostl::size_t(tail_value)
  };
};
...
template <class List, nanostl::size_t i>
...
template <class Head, class Tail, nanostl::size_t i>
...
template <nanostl::size_t A, nanostl::size_t S>
...
template <typename List, nanostl::size_t N>
struct type_of_size {
...
template <typename T>
inline nanostl::size_t hash(T const& v) {
  return nanostl::hash<T>()(v);
}

inline nanostl::size_t hash(T0 const&) { return 0; }
inline nanostl::size_t hash(T1 const&) { return 0; }
inline nanostl::size_t hash(T2 const&) { return 0; }
inline nanostl::size_t hash(T3 const&) { return 0; }
inline nanostl::size_t hash(T4 const&) { return 0; }
inline nanostl::size_t hash(T5 const&) { return 0; }
inline nanostl::size_t hash(T6 const&) { return 0; }
inline nanostl::size_t hash(T7 const&) { return 0; }
inline nanostl::size_t hash(T8 const&) { return 0; }
inline nanostl::size_t hash(T9 const&) { return 0; }
inline nanostl::size_t hash(T10 const&) { return 0; }
inline nanostl::size_t hash(T11 const&) { return 0; }
inline nanostl::size_t hash(T12 const&) { return 0; }
inline nanostl::size_t hash(T13 const&) { return 0; }
inline nanostl::size_t hash(T14 const&) { return 0; }
inline nanostl::size_t hash(T15 const&) { return 0; }
...
struct helper {
  ...
  static type_index_t to_index_t(nanostl::size_t index) {
  ...
  static type_index_t construct_t(void* data, Args&&... args) {
    new (data) T(nanostl::forward<Args>(args)...);
  ...
  static type_index_t construct_i(void* data, Args&&... args) {
    ...
    construct_t<type>(data, nanostl::forward<Args>(args)...);
  ...
  static type_index_t move_construct(type_index_t const from_index,
    ...
    switch (from_index) {
      case 0:
        new (to_value) T0(nanostl::move(*as<T0>(from_value)));
        break;
      case 1:
        new (to_value) T1(nanostl::move(*as<T1>(from_value)));
        break;
      case 2:
        new (to_value) T2(nanostl::move(*as<T2>(from_value)));
        break;
      case 3:
        new (to_value) T3(nanostl::move(*as<T3>(from_value)));
        break;
      case 4:
        new (to_value) T4(nanostl::move(*as<T4>(from_value)));
        break;
      case 5:
        new (to_value) T5(nanostl::move(*as<T5>(from_value)));
        break;
      case 6:
        new (to_value) T6(nanostl::move(*as<T6>(from_value)));
        break;
      case 7:
        new (to_value) T7(nanostl::move(*as<T7>(from_value)));
        break;
      case 8:
        new (to_value) T8(nanostl::move(*as<T8>(from_value)));
        break;
      case 9:
        new (to_value) T9(nanostl::move(*as<T9>(from_value)));
        break;
      case 10:
        new (to_value) T10(nanostl::move(*as<T10>(from_value)));
        break;
      case 11:
        new (to_value) T11(nanostl::move(*as<T11>(from_value)));
        break;
      case 12:
        new (to_value) T12(nanostl::move(*as<T12>(from_value)));
        break;
      case 13:
        new (to_value) T13(nanostl::move(*as<T13>(from_value)));
        break;
      case 14:
        new (to_value) T14(nanostl::move(*as<T14>(from_value)));
        break;
      case 15:
        new (to_value) T15(nanostl::move(*as<T15>(from_value)));
        break;
    }
    return from_index;
  }
  ...
  static type_index_t move_assign(type_index_t const from_index,
    ...
    switch (from_index) {
      case 0:
        *as<T0>(to_value) = nanostl::move(*as<T0>(from_value));
        break;
      case 1:
        *as<T1>(to_value) = nanostl::move(*as<T1>(from_value));
        break;
      case 2:
        *as<T2>(to_value) = nanostl::move(*as<T2>(from_value));
        break;
      case 3:
        *as<T3>(to_value) = nanostl::move(*as<T3>(from_value));
        break;
      case 4:
        *as<T4>(to_value) = nanostl::move(*as<T4>(from_value));
        break;
      case 5:
        *as<T5>(to_value) = nanostl::move(*as<T5>(from_value));
        break;
      case 6:
        *as<T6>(to_value) = nanostl::move(*as<T6>(from_value));
        break;
      case 7:
        *as<T7>(to_value) = nanostl::move(*as<T7>(from_value));
        break;
      case 8:
        *as<T8>(to_value) = nanostl::move(*as<T8>(from_value));
        break;
      case 9:
        *as<T9>(to_value) = nanostl::move(*as<T9>(from_value));
        break;
      case 10:
        *as<T10>(to_value) = nanostl::move(*as<T10>(from_value));
        break;
      case 11:
        *as<T11>(to_value) = nanostl::move(*as<T11>(from_value));
        break;
      case 12:
        *as<T12>(to_value) = nanostl::move(*as<T12>(from_value));
        break;
      case 13:
        *as<T13>(to_value) = nanostl::move(*as<T13>(from_value));
        break;
      case 14:
        *as<T14>(to_value) = nanostl::move(*as<T14>(from_value));
        break;
      case 15:
        *as<T15>(to_value) = nanostl::move(*as<T15>(from_value));
        break;
    }
    return from_index;
  }
...
template <class T>
constexpr nanostl::size_t variant_size_v = variant_size<T>::value;
#endif
...
template <nanostl::size_t K, class T>
struct variant_alternative; /* undefined */

template <nanostl::size_t K, class T0, class T1, class T2, class T3, class T4,
...
#if variant_CPP11_OR_GREATER
variant_constexpr nanostl::size_t variant_npos =
    static_cast<nanostl::size_t>(-1);
#else
static const nanostl::size_t variant_npos = static_cast<nanostl::size_t>(-1);
#endif
...
class bad_variant_access : public nanostl::exception {
 public:
#if variant_CPP11_OR_GREATER
...
class variant {
 ...
 public:
  ...
  variant(T1&& t1) : type_index(1) { new (ptr()) T1(nanostl::move(t1)); }
  variant(T2&& t2) : type_index(2) { new (ptr()) T2(nanostl::move(t2)); }
  variant(T3&& t3) : type_index(3) { new (ptr()) T3(nanostl::move(t3)); }
  variant(T4&& t4) : type_index(4) { new (ptr()) T4(nanostl::move(t4)); }
  variant(T5&& t5) : type_index(5) { new (ptr()) T5(nanostl::move(t5)); }
  variant(T6&& t6) : type_index(6) { new (ptr()) T6(nanostl::move(t6)); }
  variant(T7&& t7) : type_index(7) { new (ptr()) T7(nanostl::move(t7)); }
  variant(T8&& t8) : type_index(8) { new (ptr()) T8(nanostl::move(t8)); }
  variant(T9&& t9) : type_index(9) { new (ptr()) T9(nanostl::move(t9)); }
  variant(T10&& t10) : type_index(10) { new (ptr()) T10(nanostl::move(t10)); }
  variant(T11&& t11) : type_index(11) { new (ptr()) T11(nanostl::move(t11)); }
  variant(T12&& t12) : type_index(12) { new (ptr()) T12(nanostl::move(t12)); }
  ...
  variant(variant&& other) noexcept(
      nanostl::is_nothrow_move_constructible<
          T0>::value&& nanostl::is_nothrow_move_constructible<T1>::value&&
          nanostl::is_nothrow_move_constructible<
              T2>::value&& nanostl::is_nothrow_move_constructible<T3>::value&&
              nanostl::is_nothrow_move_constructible<
                  T4>::value&& nanostl::is_nothrow_move_constructible<T5>::
                  value&& nanostl::is_nothrow_move_constructible<
                      T6>::value&& nanostl::is_nothrow_move_constructible<T7>::
                      value&& nanostl::is_nothrow_move_constructible<
                          T8>::value&&
                          nanostl::is_nothrow_move_constructible<T9>::value&&
                              nanostl::is_nothrow_move_constructible<
                                  T10>::value&& nanostl::
                                  is_nothrow_move_constructible<T11>::value&&
                                      nanostl::is_nothrow_move_constructible<
                                          T12>::value&& nanostl::
  ...
  explicit variant(nonstd_lite_in_place_type_t(T), Args&&... args) {
    ...
    type_index = helper_type::template construct_t<T>(
        ptr(), nanostl::forward<Args>(args)...);
  }

  template <class T, class U,
            class... Args variant_REQUIRES_T(
                nanostl::is_constructible<T, nanostl::initializer_list<U>&,
                                          Args...>::value)>
  explicit variant(nonstd_lite_in_place_type_t(T),
                   nanostl::initializer_list<U> il, Args&&... args) {
    type_index = variant_npos_internal();
    type_index = helper_type::template construct_t<T>(
        ptr(), il, nanostl::forward<Args>(args)...);
  }

  template <nanostl::size_t K,
            class... Args variant_REQUIRES_T(
                nanostl::is_constructible<type_at_t<K>, Args...>::value)>
  ...
  template <
      nanostl::size_t K, class U,
      class... Args variant_REQUIRES_T(
          nanostl::is_constructible<type_at_t<K>, nanostl::initializer_list<U>&,
                                    Args...>::value)>
  explicit variant(nonstd_lite_in_place_index_t(K),
                   nanostl::initializer_list<U> il, Args&&... args) {
    type_index = variant_npos_internal();
    type_index = helper_type::template construct_i<K>(
        ptr(), il, nanostl::forward<Args>(args)...);
  ...
  variant& operator=(variant&& other) noexcept(
      nanostl::is_nothrow_move_assignable<
          T0>::value&& nanostl::is_nothrow_move_assignable<T1>::value&&
          nanostl::is_nothrow_move_assignable<
              T2>::value&& nanostl::is_nothrow_move_assignable<T3>::value&&
              nanostl::is_nothrow_move_assignable<
                  T4>::value&& nanostl::is_nothrow_move_assignable<T5>::value&&
                  nanostl::is_nothrow_move_assignable<
                      T6>::value&& nanostl::is_nothrow_move_assignable<T7>::
                      value&& nanostl::is_nothrow_move_assignable<T8>::value&&
                          nanostl::is_nothrow_move_assignable<T9>::value&&
                              nanostl::is_nothrow_move_assignable<T10>::value&&
                                  nanostl::is_nothrow_move_assignable<
                                      T11>::value&& nanostl::
                                      is_nothrow_move_assignable<T12>::value&&
                                          nanostl::is_nothrow_move_assignable<
                                              T13>::value&& nanostl::
                                              is_nothrow_move_assignable<
                                                  ...
                                                  is_nothrow_move_assignable<
                                                      T15>::value) {
    return move_assign(nanostl::move(other));
  }

  variant& operator=(T0&& t0) { return assign_value<0>(nanostl::move(t0)); }
  variant& operator=(T1&& t1) { return assign_value<1>(nanostl::move(t1)); }
  variant& operator=(T2&& t2) { return assign_value<2>(nanostl::move(t2)); }
  variant& operator=(T3&& t3) { return assign_value<3>(nanostl::move(t3)); }
  variant& operator=(T4&& t4) { return assign_value<4>(nanostl::move(t4)); }
  variant& operator=(T5&& t5) { return assign_value<5>(nanostl::move(t5)); }
  variant& operator=(T6&& t6) { return assign_value<6>(nanostl::move(t6)); }
  variant& operator=(T7&& t7) { return assign_value<7>(nanostl::move(t7)); }
  variant& operator=(T8&& t8) { return assign_value<8>(nanostl::move(t8)); }
  variant& operator=(T9&& t9) { return assign_value<9>(nanostl::move(t9)); }
  variant& operator=(T10&& t10) { return assign_value<10>(nanostl::move(t10)); }
  variant& operator=(T11&& t11) { return assign_value<11>(nanostl::move(t11)); }
  variant& operator=(T12&& t12) { return assign_value<12>(nanostl::move(t12)); }
  ...
  nanostl::size_t index() const {
    return variant_npos_internal() == type_index
               ? variant_npos
               : static_cast<nanostl::size_t>(type_index);
  }
  ...
  variant_alternative_t<K, variant>& emplace(Args&&... args) {
    return this->template emplace<type_at_t<K> >(
        nanostl::forward<Args>(args)...);
  }
  ...
  template <
      ...
      class... Args variant_REQUIRES_T(
          nanostl::is_constructible<type_at_t<K>, nanostl::initializer_list<U>&,
                                    Args...>::value)>
  variant_alternative_t<K, variant>& emplace(nanostl::initializer_list<U> il,
                                             Args&&... args) {
    return this->template emplace<type_at_t<K> >(
        il, nanostl::forward<Args>(args)...);
  }
...
#if variant_CPP11_OR_GREATER
      noexcept(
          nanostl::is_nothrow_move_constructible<T0>::value&& std17::is_nothrow_swappable<
              T0>::value&& nanostl::is_nothrow_move_constructible<T1>::value&& std17::
              is_nothrow_swappable<T1>::value&& nanostl::is_nothrow_move_constructible<
                  T2>::value&& std17::is_nothrow_swappable<T2>::value&& nanostl::
                  is_nothrow_move_constructible<T3>::value&& std17::is_nothrow_swappable<
                      T3>::value&& nanostl::is_nothrow_move_constructible<T4>::value&&
                      std17::is_nothrow_swappable<T4>::value&& nanostl::is_nothrow_move_constructible<
                          T5>::value&& std17::is_nothrow_swappable<T5>::value&& nanostl::
                          is_nothrow_move_constructible<T6>::value&& std17::is_nothrow_swappable<
                              T6>::value&& nanostl::is_nothrow_move_constructible<T7>::value&&
                              std17::is_nothrow_swappable<T7>::value&& nanostl::is_nothrow_move_constructible<
                                  T8>::value&& std17::is_nothrow_swappable<T8>::
                                  value&& nanostl::is_nothrow_move_constructible<
                                      T9>::value&& std17::is_nothrow_swappable<T9>::
                                      value&& nanostl::is_nothrow_move_constructible<
                                          T10>::value&& std17::is_nothrow_swappable<T10>::
                                          value&& nanostl::is_nothrow_move_constructible<
...
#if variant_CPP11_OR_GREATER
      variant tmp(nanostl::move(*this));
      *this = nanostl::move(other);
      other = nanostl::move(tmp);
#else
      variant tmp(*this);
      *this = other;
      other = tmp;
  ...
  template <class T>
  static variant_constexpr nanostl::size_t index_of() variant_noexcept {
    return to_size_t(
 ...
 private:
  ...
  template <class U>
  static variant_constexpr nanostl::size_t to_size_t(U index) {
    return static_cast<nanostl::size_t>(index);
  }
  ...
  variant& assign_value(T&& value) {
    if (index() == K) {
      *as<T>() = nanostl::forward<T>(value);
    } else {
      helper_type::destroy(type_index, ptr());
      type_index = variant_npos_internal();
      new (ptr()) T(nanostl::forward<T>(value));
      type_index = K;
    }
    return *this;
  }
 ...
 private:
  ...
  using aligned_storage_t =
      typename nanostl::aligned_storage<data_size, data_align>::type;
...
inline R&& get(variant<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12,
                       T13, T14, T15>&& v,
               nonstd_lite_in_place_type_t(R) = nonstd_lite_in_place_type(R)) {
  return nanostl::move(v.template get<R>());
}
...
#endif
  return nanostl::move(v.template get<K>());
}
...
#if variant_CPP11_OR_GREATER
        variant_REQUIRES_T(
            nanostl::is_move_constructible<T0>::value&& std17::is_swappable<
                T0>::value&& nanostl::is_move_constructible<T1>::value&&
                std17::is_swappable<T1>::value&& nanostl::is_move_constructible<
                    T2>::value&& std17::is_swappable<T2>::value&& nanostl::
                    is_move_constructible<T3>::value&& std17::is_swappable<
                        T3>::value&& nanostl::is_move_constructible<T4>::value&&
                        std17::is_swappable<T4>::value&& nanostl::is_move_constructible<
                            T5>::value&& std17::is_swappable<T5>::value&& nanostl::
                            is_move_constructible<T6>::value&& std17::is_swappable<
                                T6>::value&& nanostl::is_move_constructible<T7>::value&&
                                std17::is_swappable<T7>::value&& nanostl::is_move_constructible<
                                    T8>::value&& std17::is_swappable<T8>::
                                    value&& nanostl::is_move_constructible<
                                        T9>::value&& std17::is_swappable<T9>::
                                        value&& nanostl::is_move_constructible<
                                            T10>::value&& std17::is_swappable<T10>::
                                            value&& nanostl::is_move_constructible<
                                                T11>::value&& std17::is_swappable<T11>::
                                                value&& nanostl::is_move_constructible<
                                                    T12>::value&&
                                                    std17::is_swappable<
                                                        T12>::value&& nanostl::
...
#else
template <nanostl::size_t NumVars, typename R, typename Visitor, typename T1,
...
#if variant_CPP11_OR_GREATER
    typedef typename variant_alternative<
        Idx, typename nanostl::decay<V1>::type>::type value_type;
#else
...
struct VisitorImpl {
  typedef decltype(nanostl::declval<Visitor>()(
      get<0>(static_cast<const V&>(nanostl::declval<V>()))...)) result_type;
...
struct hash<nonstd::variant<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11,
  ...
  nanostl::size_t operator()(
    ...
    switch (v.index()) {
      case 0:
        return nvd::hash(0) ^ nvd::hash(this->get<0>(v));
      case 1:
        return nvd::hash(1) ^ nvd::hash(this->get<1>(v));
      case 2:
        return nvd::hash(2) ^ nvd::hash(this->get<2>(v));
      case 3:
        return nvd::hash(3) ^ nvd::hash(this->get<3>(v));
      case 4:
        return nvd::hash(4) ^ nvd::hash(this->get<4>(v));
      case 5:
        return nvd::hash(5) ^ nvd::hash(this->get<5>(v));
      case 6:
        return nvd::hash(6) ^ nvd::hash(this->get<6>(v));
      case 7:
        return nvd::hash(7) ^ nvd::hash(this->get<7>(v));
      case 8:
        return nvd::hash(8) ^ nvd::hash(this->get<8>(v));
      case 9:
        return nvd::hash(9) ^ nvd::hash(this->get<9>(v));
      case 10:
        return nvd::hash(10) ^ nvd::hash(this->get<10>(v));
      case 11:
        return nvd::hash(11) ^ nvd::hash(this->get<11>(v));
      case 12:
        return nvd::hash(12) ^ nvd::hash(this->get<12>(v));
      case 13:
        return nvd::hash(13) ^ nvd::hash(this->get<13>(v));
      case 14:
        return nvd::hash(14) ^ nvd::hash(this->get<14>(v));
      case 15:
        return nvd::hash(15) ^ nvd::hash(this->get<15>(v));
