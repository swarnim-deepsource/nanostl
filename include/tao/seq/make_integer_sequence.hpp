...
struct generate_sequence<false, false> {
  template <typename T, T M, T N, nanostl::size_t S, T... Ns>
  using f =
      typename generate_sequence<(N & (M / 2)) != 0, (M / 2) == 0>::template f<
          T, M / 2, N, 2 * S, Ns..., (Ns + S)...>;
};
...
struct generate_sequence<true, false> {
  ...
  using f =
      typename generate_sequence<(N & (M / 2)) != 0, (M / 2) == 0>::template f<
          T, M / 2, N, 2 * S + 1, Ns..., (Ns + S)..., 2 * S>;
};

// the final sequence per T/N should be memoized, it will probably be used
// multiple times. also checks the limit and starts the above generator
// properly.
